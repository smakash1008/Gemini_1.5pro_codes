...
prompt = "Describe the animals in this video and its actions. Give all other details other than that in points about 5 to 10 lines."

def get_response(file_uploaded,prompt):
    response = model.generate_content([file_uploaded,prompt],request_options={"retry": retry.Retry(predicate=retry.if_transient_error)})
    return response.text

data = get_response(file_uploaded,prompt)
data = data.replace("**","")
data = data.replace("*","")
data = data.replace("#","")
print(data)
