...
instruction = "Behave like a good AI system and Follow the Prompt clearly and Give the Final data in a detailed way."
...
async def get_response2():
    async for chunk in await model.generate_content_async("Tell me a story about Cheras",stream=True, request_options={'retry' : retry_async.AsyncRetry(predicate=retry_async.if_transient_error)}):
        if chunk.text:
            print(chunk.text)
        print("#"*100)

async def get_response3():
...
async def gather_response():
    ...
    task4 = asyncio.create_task(get_response3())
