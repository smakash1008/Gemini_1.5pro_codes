...
async def get_response(prompt):
    async for chunk in await model.generate_content_async(prompt, stream=True, request_options={'retry' : retry_async.AsyncRetry(predicate=retry_async.if_transient_error)}):
        if chunk.text:
            print(chunk.text)
        print("$"*100)

asyncio.run(get_response(prompt))